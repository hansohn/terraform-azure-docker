...
# ad-hoc builds, e.g. AZURE_CLI_VERSION=2.88.0 make docker/build.
dockerfile-arg = $(shell grep -E '^ARG $(1)=' Dockerfile | head -1 | cut -d= -f2)
TERRAFORM_VERSION       ?= $(call dockerfile-arg,TERRAFORM_VERSION)
TFLINT_AZURERM_VERSION  ?= $(call dockerfile-arg,TFLINT_AZURERM_VERSION)
AZURE_CLI_VERSION       ?= $(call dockerfile-arg,AZURE_CLI_VERSION)
...
DOCKER_BUILD_ARGS += --build-arg TFLINT_AZURERM_VERSION=$(TFLINT_AZURERM_VERSION)
DOCKER_BUILD_ARGS += --build-arg AZURE_CLI_VERSION=$(AZURE_CLI_VERSION)
